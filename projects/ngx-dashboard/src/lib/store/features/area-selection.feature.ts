...
export interface AreaSelectionState {
   ...
   * kept afterwards: the user has to be able to see what they selected before
 ...
 *
 * Only the rectangle lives here. Which widgets it catches is a question about
 * `cells`, which belongs to another feature, so the store composes the two —
 * the same split the drag and resize features already use.
 */
  ...
  signalStoreFeature(
    ...
    withMethods((store) => {
      ...
      return {
         ...
         *  highlight would otherwise outlive the only UI that can act on it. */
